...
namespace Utility
{
    ...
    public class RegExp
    {
        ...
        /// <summary>
        /// 是否是手机号  匹配格式：11位手机号码,3-4位区号，7-8位直播号码，1－4位分机号如：12345678901、1234-12345678-1234
        /// </summary>
        /// <param name="s"></param>
        /// <param name="isRestrict">是否按严格格式验证</param>
        /// <returns></returns>
        public static bool IsMobileNo(string s)
        {
            return Regex.IsMatch(s, @"((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)", RegexOptions.IgnoreCase);
        }
    }
}
